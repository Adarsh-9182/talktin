      ...
      {shown.length === 0 ? (
        ...
        <ul className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {shown.map((voice) => {
            ...
            return (
              <li key={voice.id} className="flex items-center gap-2 rounded-2xl border border-line bg-surface pr-3 transition-colors hover:border-ink/25">
                <button
                ...
                <Link
                  href={`/voices/${voice.id.toLowerCase()}`}
                  aria-label={`About ${voice.name}`}
